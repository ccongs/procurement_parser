...
APScheduler BackgroundScheduler 로 수집을 주기 실행한다. 매 tick 은 한 흐름이다:
① 게이트 확인(enabled/auto_halted) → ② 수집 윈도우 산정(last_success_dt 기반) →
③ collector.collect_window(trigger="scheduled") 호출.

수집/페이징/dedup/upsert/재시도/halt/collection_run 기록/last_success_dt 갱신은 전부
collect_window 안에 있다. 스케줄러는 **"언제·어떤 윈도우로 collect_window 를 부를지"만**
책임진다(수집 로직을 재구현하지 않는다).
...
def start_scheduler(run_now: bool = False) -> BackgroundScheduler:
    ...
    logger.info(
        "스케줄러 시작: interval=%d분 run_now=%s next_run=%s",
        interval_minutes,
        run_now,
...
def reschedule(interval_minutes: int) -> None:
    """스케줄러가 실행 중이면 잡 트리거를 새 interval 로 교체. 아니면 무시.
    ...
    _scheduler.reschedule_job(
        _JOB_ID, trigger=IntervalTrigger(minutes=interval_minutes)
    )
    job = _scheduler.get_job(_JOB_ID)
    logger.info(
        "스케줄러 재스케줄: interval=%d분 next_run=%s",
        interval_minutes,
        getattr(job, "next_run_time", None),
    )
